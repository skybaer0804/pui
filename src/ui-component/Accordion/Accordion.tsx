...
export function Accordion({
    ...
    const [uncontrolledExpanded, setUncontrolledExpanded] = useState<AccordionValue[]>(normalizeExpanded(defaultExpanded, allowMultiple));
    ...
    const classes = ['accordion', disableGutters ? 'accordion--no-gutters' : '', allowMultiple ? 'accordion--multiple' : 'accordion--single', className]
        .filter(Boolean)
        .join(' ');
            ...
            {items.map((item, index) => {
                ...
                return (
                    <div
                        ...
                        className={['accordion__item', isOpen ? 'accordion__item--expanded' : '', isDisabled ? 'accordion__item--disabled' : '']
                            .filter(Boolean)
                            .join(' ')}
                    >
                        <button
                        ...
                        <div id={detailsId} className="accordion__details" role="region" aria-labelledby={summaryId} hidden={!isOpen}>
                            {item.details}
                        </div>
                    </div>
                );
