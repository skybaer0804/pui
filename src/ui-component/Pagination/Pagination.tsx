...
const createItems = (args: {
    ...
    const { count, page, siblingCount, boundaryCount, showFirstButton, showLastButton, hidePrevButton, hideNextButton, disabled } = args;
    ...
    const siblingsEnd = Math.min(Math.max(page + siblingCount, boundaryCount + siblingCount * 2 + 2), endPages.length ? endPages[0] - 2 : count - 1);
...
export function Pagination({
    ...
    const items = useMemo(
        ...
        [safeCount, currentPage, siblingCount, boundaryCount, showFirstButton, showLastButton, hidePrevButton, hideNextButton, disabled]
    );

    if (safeCount <= 1) return null;

    const classes = ['pagination', `pagination--${variant}`, `pagination--${shape}`, `pagination--${size}`, disabled ? 'pagination--disabled' : '', className]
        .filter(Boolean)
        .join(' ');
